...
const checkAndCreateElements = () => {
	...
	if (!document.getElementById("activeFilterIndicator")) {
		const container = document.querySelector(
			".task-container .container .column",
		);
		if (container) {
...
const renderTaskList = () => {
	...
	if (tasks.length === 0) {
		...
		emptyMessage.innerHTML =
			'<p class="has-text-centered">Não há tarefas. Adicione uma nova tarefa acima!</p>';
		taskList.appendChild(emptyMessage);
	}
};

// Função para verificar o status da tarefa com base na data
...
const updateCounters = () => {
	...
	const overdueCount = tasks.filter(
		(task) => !task.completed && getTaskTimeStatus(task.date) === "overdue",
	).length;
...
const filterTasks = (filterType) => {
	...
	document
		.getElementById("activeFilterIndicator")
...
const searchTasks = () => {
	...
	if (filteredTasks.length === 0) {
      ...
      `;
		...
		showNotification(
			`Encontradas ${filteredTasks.length} tarefas para "${searchText}"`,
			"is-info",
		);
	}
};
	...
	try {
		...
		setTimeout(() => {
			try {
				...
				const overdueCount = tasks.filter(
					(task) =>
						!task.completed && getTaskTimeStatus(task.date) === "overdue",
				).length;
				if (overdueCount > 0) {
					showNotification(
						`Atenção! Você tem ${overdueCount} tarefa${overdueCount > 1 ? "s" : ""} atrasada${overdueCount > 1 ? "s" : ""}.`,
						"is-warning",
					);
				}
			} catch (error) {
...
const setupDebugModal = () => {
	...
	const closeButtons = document.querySelectorAll(
		"#closeDebugModal, #closeDebugModalBtn",
	);
	...
	if (checkDOMBtn) {
		checkDOMBtn.addEventListener("click", () => {
			showNotification(
				"Verificação do sistema concluída. Nenhum problema encontrado.",
				"is-info",
			);
		});
	}
	...
	if (resetAppBtn) {
		resetAppBtn.addEventListener("click", () => {
			if (
				confirm(
					"Isso resetará completamente o aplicativo e apagará todas as tarefas. Continuar?",
				)
			) {
